import { Vector3 } from "../../../yuka/src/math/Vector3.js";
import { Polygon } from "../../../yuka/src/math/Polygon.js";
import { HalfEdge } from "../../../yuka/src/math/HalfEdge.js";
import { AABB } from "../../../yuka/src/math/AABB.js";
import { LineSegment } from "../../../yuka/src/math/LineSegment.js";
import { Plane } from "../../../yuka/src/math/Plane.js";
// import { CellSpacePartitioning } from "../../../yuka/src/partitioning/CellSpacePartitioning.js";
import { BVH } from "../../../yuka/src/math/BVH.js";
...
import {NavMeshUtils} from "../../../yuka/src/navigation/navmesh/NavMeshUtils.js";

import {MathUtils} from "../../../yuka/src/math/MathUtils.js";

import {Dijkstra} from "../../../yuka/src/graph/search/Dijkstra.js";
import { NavEdge } from "../../../yuka/src/navigation/core/NavEdge.js";
...
function enclosedSegments(segments) {
	...
	let alpha = segments[0];
	alpha = alpha[0];
	return segments.concat([omega, alpha]);
}
...
function polygonToReverseCell(polygon) {
	let edge = polygon.edge;
	let arr = [];
	do {
		arr.push([edge.vertex.x, edge.vertex.z]);
...
function getTriPolygon(vertSoup, tri, y) {
	...
	let poly = new Polygon().fromContour([
		new Vector3(vertSoup[tri[2]][0], y, vertSoup[tri[2]][1]),
		new Vector3(vertSoup[tri[1]][0], y, vertSoup[tri[1]][1]),
		new Vector3(vertSoup[tri[0]][0], y, vertSoup[tri[0]][1])
	]);
	return poly;
}
...
class SVGCityReader {
	...
	getNavmeshExtrudedGeometry() {
		...
		*/
		...
		if (this.navmeshRoad) {
			// Highways
			let navmesh = this.navmeshHighways = this.getNavmeshHighways();
	...
	getNavmeshVerticesAndEdges(navmesh, vertexArr=null, edgeConstraints=null) {
		if (vertexArr === null) {
			vertexArr = [];
		}
		...
		navmesh._borderEdges.forEach((edge) => {
			...
			edgeConstraints.push([edge.prev.vertex.index, edge.vertex.index]);
		});
	...
	buildCityTowerRoofNavmesh(inset=0, minChamferDist=1e-5, asGeometry=false) {
		const scaleXZ = this._PREVIEW_MODE ? 1 : this.exportScaleXZ;
		let svg;
		if (this._PREVIEW_MODE) {
			svg = $(this.makeSVG("g", {}));
	...
	buildBuildingRoofsNavmeshes(inset=0, minChamferDist=1e-5, asGeometry=false ) {
		const scaleMult = this._PREVIEW_MODE ? 1/this.exportScaleXZ : 1;
		let svg;
		if (this._PREVIEW_MODE) {
			svg = $(this.makeSVG("g", {}));
			this.map.append(svg, {});
		...
		if (this._PREVIEW_MODE) {
			navmeshes = navmeshes.map((n)=>NavMeshUtils.getNewScaledNavmesh(n, scaleMult, 1, scaleMult));
		...
		return navmeshes.map((n) => NavMeshUtils.collectPlainNavmeshGeometry(null, n.regions));
	}
	...
	buildGroundNavmesh(inset=0, minChamferDist=1e-5) {
		...
		this.profileWardBuildings.forEach((ward)=> {
			ward.forEach((b)=> {
				...
				if (buildingPoints.length < 3) throw new Error("not enough points for polygon!");
				// else if (!new Polygon().fromContour(buildingPoints.map(pointToVec)).convex(false)) console.warn("invalid polygon?");
			});
		});
		...
		for (let i = 0; i< this.cityWallCDTObj.vertices.length; i++) {
			this.cityWallCDTObj.vertices[i][0] *= scaleXZ;
			this.cityWallCDTObj.vertices[i][1] *= scaleXZ;
		}
		...
		// clean up overlaping triangle cdt polygons over navmesh city wall
		let navmeshCityWall = NavMeshUtils.getNewScaledNavmesh(this.navmeshCityWallWalk, scaleXZ, 1, scaleXZ);
		let navmeshCityWallBVH = new BVH(2, 1, 10).fromMeshGeometry(NavMeshUtils.getMeshGeometryFromPolygons(navmeshCityWall.regions));
		cdt = cdt.filter((c)=> {
		...
		const INITIAL_PREVIEW = false;

		if (this._PREVIEW_MODE) {
			svg = $(this.makeSVG("g", {}));
			this.map.append(svg, {});
			svg.append(this.makeSVG("path", {fill:"rgba(55,255,0,"+(INITIAL_PREVIEW ? 0.9 : 0.3)+")", stroke:"blue", "stroke-width": (INITIAL_PREVIEW ? 0.15 : 0), d: navmesh.regions.map(polygonSVGString).join(" ") }));
		...
		if (this.navmeshRoad) {
			rampShadowPolygons = NavMeshUtils.scalePolygons(NavMeshUtils.seperateMarkedPolygonsVertices(NavMeshUtils.filterOutPolygonsByMask(this.navmeshRoad.regions, BIT_HIGHWAY_RAMP, true), false), scaleXZ, 1, scaleXZ);
		...
		if (inset !== 0) {
			...
			let resultMap = NavMeshUtils.getBorderEdgeOffsetProjections(navmesh, inset, minChamferDist);
			if (INITIAL_PREVIEW && this._PREVIEW_MODE) { // switchero initial testing versus default implementation
				svg = $(this.makeSVG("g", {}));
				this.map.append(svg, {});
				...
				/*
					,this._PREVIEW_MODE ? (edge) => {svg.append(this.makeSVG("path", {fill:"none", stroke:"orange", "stroke-width": 0.15, d:[[edge.prev.vertex, edge.vertex], [edge.value.from, edge.value.to]].map((e)=>lineSegmentSVGStr(e[0], e[1])).join(" ")}));}
						: null,
					this._PREVIEW_MODE ? (theChamfer) => {svg.append(this.makeSVG("path", {fill:"none", stroke:"red", "stroke-width": 0.15, d:edges.map((e) => lineSegmentSVGStr(theChamfer.from, theChamfer.to)).join(" ")}))}
						: null
				*/
				);
				// return navmesh;
		...
		this.wards.forEach((wardObj)=> {
			...
			wardObj.neighborhoodPts.forEach((buildingsList)=> {
				buildingsList.forEach((building)=>{
						...
						let increase =  BASE_BUILDING_HEIGHT_RANGE * PRNG.nextFloat();
	...
	static buildBasicQuadRoofs(wardCollector, wardRoofCollector, buildingTopIndices, vertexNormals, buildingInset) {
		if (buildingTopIndices.length !== 4 ||  PRNG.nextFloat() > this.quadRoofChance) {
	...
	testSubdivideBuilding(building) {
		...
		*/
		...
		//edges[Math.floor( PRNG.nextFloat() * edgeCount)
