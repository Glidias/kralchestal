...
export function _sampleTriFromRegion(triSample:TriSample, region:Polygon, regionArea:number, fromIndex:number, toIndex:number, r:number) {
	r *= regionArea;
...
export type TriAreaCallback = (area:number, a:Vector3, b:Vector3, c:Vector3)=>number;

export function getArea2DOfPolygon(polygon:Polygon, triAreaCallback:TriAreaCallback=null):number {
	let w:HalfEdge;
	var a:Vector3 = polygon.edge.vertex;
	var areaAccum:number = 0;
	w = polygon.edge;
	var wn:HalfEdge = w.next;
	do  {
		var b:Vector3 = w.vertex;
		var c:Vector3 = wn.vertex;
		let areaToAdd = (( c.x - a.x ) * ( b.z - a.z ) - ( b.x - a.x ) * ( c.z - a.z )) * 0.5;;
		areaAccum +=  triAreaCallback === null ? areaToAdd : triAreaCallback(areaToAdd, a, b, c);
		w = w.next;
		wn = wn.next;
	} while(wn !== polygon.edge)
	return areaAccum;
}

export function get3DSurfaceAreaPolygon(polygon:Polygon, triAreaCallback:TriAreaCallback=null):number {
	let w:HalfEdge;
	var a:Vector3 = polygon.edge.vertex;
	var areaAccum:number = 0;
	w = polygon.edge;
	...
	do  {
		...
		let areaToAdd = Math.sqrt(cx * cx + cy * cy + cz * cz) * 0.5;
		areaAccum += triAreaCallback === null ? areaToAdd : triAreaCallback(areaToAdd, a, b, c);
		w = w.next;
		wn = wn.next;
	} while(wn !== polygon.edge)
...
// Trace methods for internal debug checking...
